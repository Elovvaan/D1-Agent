        ...
        <div className="mx-auto max-w-[1200px]">
          ...
          <section className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
            <form className="rounded-[30px] border border-white/12 bg-[#050505] p-6">
              ...
              <div className="mt-8 border-t border-white/10 pt-6"><p className="text-xs font-black uppercase tracking-[0.24em] text-[#8CFF00]">Event</p><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-black text-white"><span>Published Event</span><select className="min-h-12 rounded-xl border border-[#8CFF00]/35 bg-black px-4 text-sm font-black text-white outline-none" name="event" disabled={activeEvents.length === 0}>{activeEvents.length > 0 ? activeEvents.map((event, index) => <option key={event.id ?? `${eventLabel(event)}-${index}`} value={event.id ?? `${eventLabel(event)}-${index}`}>{eventLabel(event)}</option>) : <option>No active events published</option>}</select></label><label className="grid gap-2 text-sm font-black text-white"><span>Entry Fee</span><input className="min-h-12 rounded-xl border border-white/10 bg-black px-4 text-sm font-black text-[#8CFF00] outline-none" readOnly value={entryFeeLabel} /></label></div></div>
