...
async function appendUserState(fileName: string, entry: Record<string, unknown>) {
  ...
  await writeFile(filePath, `${JSON.stringify({ items: [entry, ...items].slice(0, 300) }, null, 2)}\n`, "utf8");
}
...
export async function signInOperator(formData: FormData) {
  ...
  if (!configuredCode) redirect("/operations?status=operator-code-missing");
  ...
  cookieStore.set(OPERATOR_COOKIE, OPERATOR_COOKIE_VALUE, { httpOnly: true, sameSite: "lax", secure: process.env.NODE_ENV === "production", path: "/operations", maxAge: 60 * 60 * 8 });
...
export async function recordSupportIssue(formData: FormData) {
  ...
  await appendUserState(OPERATOR_ISSUES_FILE, { id: `issue-${randomUUID()}`, subject, affectedArea, accountType, detail, severity, status: "open", createdAt: new Date().toISOString() });
...
export async function recordInboundMessage(formData: FormData) {
  ...
  await appendUserState(OPERATOR_INBOX_FILE, { id: `inbound-${randomUUID()}`, senderName, senderContact, source, subject, body, status: "open", receivedAt: new Date().toISOString() });
...
export async function recordViewAsUser(formData: FormData) {
  const userId = value(formData, "userId") || "athlete-current";
...
export async function recordBuildRoomRequest(formData: FormData) {
  ...
  await appendUserState(OPERATOR_ISSUES_FILE, { id: `build-${randomUUID()}`, subject: area || "Build request", affectedArea: area, accountType: "platform", detail: request, severity: "build-room", status: "open", createdAt: new Date().toISOString() });
...
export async function recordDataIntake(formData: FormData) {
  ...
  const athleteText = value(formData, "athleteText");
  ...
  await audit("data-intake-recorded", { sourceType, state, district, school, sport, sourceUrl, pdfName: pdfMeta?.name });
