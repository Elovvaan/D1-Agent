...
const tools: Array<{ label: string; href: string; detail: string; icon: LucideIcon }> = [
  ...
  { label: "NCES Sync", href: "/operations/nces", detail: "Auto school directory", icon: Database },
...
export default async function OperationsCommandPage({ searchParams }: { searchParams?: Promise<{ tab?: string; state?: string; status?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#061331] px-4 py-6 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-[1800px]"><header className="rounded-[30px] border border-white/10 bg-[#0A1A3F] p-5"><p className="text-xs font-black uppercase tracking-[0.24em] text-[#F2C200]">MyD1 Command Window</p><h1 className="mt-1 text-3xl font-black">Operations Center</h1><p className="mt-1 text-sm font-semibold text-[#CAD7FF]">Top tabs edit global pages. Left states edit state profiles. Each workflow stays separate.</p>{params.status ? <div className="mt-3 rounded-2xl border border-[#F2C200]/30 bg-[#F2C200]/10 px-4 py-3 text-sm font-black text-[#FFE27A]">Saved</div> : null}<nav className="mt-5 flex flex-wrap gap-2">{tabs.map((tab) => <Link key={tab.id} href={`/operations?tab=${tab.id}`} className={`rounded-2xl px-4 py-2 text-sm font-black ${isPageWorkflow && activeTab === tab.id ? "bg-[#F2C200] text-[#061331]" : "bg-white/[0.08] text-white"}`}>{tab.label}</Link>)}<Link href="/operations/nces" className="rounded-2xl bg-white/[0.08] px-4 py-2 text-sm font-black text-white">NCES Sync</Link><Link href="/operations/games" className="rounded-2xl bg-[#F2C200] px-4 py-2 text-sm font-black text-[#061331]">Game Intake</Link></nav></header><section className="mt-5 rounded-[28px] border border-white/10 bg-white/[0.06] p-4"><div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-center"><div className="flex items-center gap-3"><Search className="text-[#F2C200]" /><h2 className="text-xl font-black">Find and edit the public page, state, school, athlete, parent, coach, recruiter, media partner, game, NCES sync, review, or tool.</h2></div><input className="min-h-12 rounded-2xl bg-white px-4 text-sm font-black text-[#061331] outline-none" placeholder="Search operations" /></div></section><div className="mt-5 grid gap-6 xl:grid-cols-[260px_1fr_300px]"><StateRail activeCode={isPageWorkflow ? undefined : selectedState.code} liveCounts={liveCounts} /><section className="min-w-0 rounded-[34px] border border-white/10 bg-white/[0.07] p-5">{isPageWorkflow ? <><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-[#F2C200]">Global page workflow</p><h2 className="mt-2 text-3xl font-black">{formatTitle(activeTab)} page workstation</h2><p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#CAD7FF]">This edits the {formatTitle(activeTab)} page only. Preview stays inside Operations.</p></div><PreviewLink href={activeTab === "home" ? "/" : `/${activeTab}`} back={activeBack} /></div><div className="mt-5 grid gap-4 md:grid-cols-3"><Stat icon={ImageIcon} label="Page media" value="Upload" /><Stat icon={FileSpreadsheet} label="Content blocks" value="Global" /><Stat icon={ShieldCheck} label="Review gate" value="Required" /></div><div className="mt-6"><PageEditor pageKey={activeTab} /></div></> : <><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-[#F2C200]">State workflow · {selectedState.code}</p><h2 className="mt-2 text-3xl font-black">{selectedState.name} state workstation</h2><p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#CAD7FF]">This edits {selectedState.name} only. Preview stays inside Operations.</p></div><PreviewLink href={`/schools/${selectedStateSlug}`} back={activeBack} /></div><div className="mt-5 grid gap-4 md:grid-cols-4"><Stat icon={Home} label="Schools" value={schoolCount} /><Stat icon={Users} label="Teams" value={teamCount} /><Stat icon={Trophy} label="Athletes" value={athleteCount} /><Stat icon={UserRound} label="Coaches" value={coachCount} /></div><div className="mt-6 grid gap-5"><StateEditor code={selectedState.code} name={selectedState.name} /><div className="grid gap-4 md:grid-cols-3"><Stat icon={FileSpreadsheet} label="State intake" value={activeIntake.length} /><Stat icon={ImageIcon} label="State media" value="Upload" /><Stat icon={ShieldCheck} label="Review gate" value="Required" /></div></div></>}</section><ToolRail /></div></div></main>;
}
