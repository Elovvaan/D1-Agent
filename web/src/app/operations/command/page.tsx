...
import { savePageProfile, saveStateProfile } from "../actions";
...
function StateEditor({ stateCode, stateName, activeTab }: { stateCode: string; stateName: string; activeTab: CommandTab }) {
  ...
  return (
    <form action={saveStateProfile} className="rounded-[30px] border border-white/12 bg-[#071A43] p-5">
      <input type="hidden" name="stateCode" value={stateCode} /><input type="hidden" name="returnTab" value={activeTab} />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-[#F2C200]">State profile manager</p><h3 className="mt-1 text-2xl font-black">Edit {stateName}</h3></div><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-[#F2C200] px-5 text-sm font-black text-[#061331]" type="submit"><Save size={16} /> Save state</button></div>
      ...
      <label className="mt-4 grid gap-2 text-sm font-black text-white"><span>Bio</span><textarea className="min-h-32 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-[#061331] outline-none" name="bio" defaultValue={profile?.bio ?? ""} placeholder="Write the state-level public directory description here." /></label>
    </form>
  );
}
...
function PageEditor({ pageKey, stateCode }: { pageKey: CommandTab; stateCode: string }) {
  ...
  return (
    <form action={savePageProfile} className="rounded-[30px] border border-white/12 bg-[#071A43] p-5">
      <input type="hidden" name="pageKey" value={pageKey} /><input type="hidden" name="stateCode" value={stateCode} />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-[#F2C200]">Public page manager</p><h3 className="mt-1 text-2xl font-black">Edit {titleCase(pageKey)} page slot</h3></div><button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-[#F2C200] px-5 text-sm font-black text-[#061331]" type="submit"><Save size={16} /> Save page</button></div>
      ...
      <label className="mt-4 grid gap-2 text-sm font-black text-white"><span>Body / public copy</span><textarea className="min-h-32 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-[#061331] outline-none" name="body" defaultValue={profile?.body ?? ""} placeholder="Write the text/content for this public page slot here." /></label>
    </form>
  );
}
  ...
  return (
    <main className="min-h-screen bg-[#061331] px-4 py-6 text-white sm:px-6 lg:px-8">
      ...
      <div className="mx-auto max-w-[1800px]">
        <header className="rounded-[30px] border border-white/10 bg-[#0A1A3F] p-4 lg:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-[#F2C200]">MyD1 Command Window</p><h1 className="mt-1 text-3xl font-black tracking-tight">Operations Center</h1><p className="mt-1 text-sm font-semibold text-[#CAD7FF]">Pick any state. Pick a public page. Edit profile photos, media, bio, and copy in the middle workstation.</p></div>{params.status ? <span className="rounded-2xl border border-[#F2C200]/30 bg-[#F2C200]/10 px-4 py-3 text-sm font-black text-[#FFE27A]">Saved</span> : null}</div><nav className="mt-5 flex flex-wrap gap-2">{pageTabs.map((tab) => <Link key={tab.id} href={`${tab.href}&state=${selectedStateIndex.code}`} className={`rounded-2xl px-4 py-2 text-sm font-black transition ${activeTab === tab.id ? "bg-[#F2C200] text-[#061331]" : "bg-white/[0.08] text-white hover:bg-white/[0.12]"}`}>{tab.label}</Link>)}</nav></header>
