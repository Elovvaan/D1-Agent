    ...
    <section className="mt-6 rounded-[28px] border border-white/10 bg-white/[0.08] p-5 shadow-[0_24px_70px_rgba(0,0,0,0.22)]" id="communications">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-[#CAD7FF]">This is the private inbox for people trying to reach you. Full Gmail/Facebook automation can connect here; for now, messages can be captured and tracked directly in Operations.</p>
        </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <form action={recordInboundMessage} className="grid gap-3 rounded-2xl border border-white/10 bg-[#071A43] p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-black text-white">Source<select className="rounded-2xl border border-white/10 bg-white px-4 py-3 text-sm font-black text-[#061331]" name="source" defaultValue="email"><option value="email">Email</option><option value="facebook">Facebook</option><option value="website">Website</option><option value="instagram">Instagram</option><option value="phone">Phone</option><option value="other">Other</option></select></label>
