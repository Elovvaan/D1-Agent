...
export default async function StateSchoolsPage({ params }: { params: Promise<{ stateCode: string }> }) {
  ...
  const tagline = profile?.tagline || `${displayName} schools`;
  return (
          ...
          <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
            ...
            <div className="min-w-0">
              <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div className="flex items-start gap-4">
                  {profile?.badgeImageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={profile.badgeImageUrl} alt={`${displayName} badge`} className="h-20 w-20 rounded-2xl border border-white/15 bg-[#FAFBFD] object-contain p-2" />
                  ) : null}
                  <div>
              ...
              <form className="grid gap-3 rounded-[26px] border border-white/12 bg-white/[0.055] p-4 backdrop-blur md:grid-cols-[1fr_auto]">
                <div className="flex min-h-12 items-center gap-3 rounded-xl bg-white px-4 text-[#0A1A3F]"><Search size={18} className="text-[#66718F]" /><input className="flex-1 bg-transparent text-sm font-semibold outline-none" name="q" placeholder={`Search ${displayName} schools...`} type="search" /></div>
              ...
              <div className="mt-8 rounded-[30px] border border-white/12 bg-white/[0.045] p-5">
                <div className="mb-5"><p className="text-xs font-black uppercase tracking-[0.22em] text-[#F2C200]">{state.code} folder</p><h2 className="mt-1 text-2xl font-black">Schools in {displayName}</h2></div>
