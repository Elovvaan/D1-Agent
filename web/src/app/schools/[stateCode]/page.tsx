...
function SchoolTile({ school }: { school: SchoolNode }) {
  return (
    <a href={school.href} className="group rounded-[32px] border border-white/12 bg-white/[0.06] p-4 backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-[#F2C200]/70 hover:bg-white/[0.09] hover:shadow-[0_26px_80px_rgba(0,0,0,0.28)]">
        ...
        <div className="relative mx-auto max-w-[1440px] px-4 py-14 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-[#C8D6FF]">Same badge-grid flow: pick a school, then move into teams, coaches, athletes, and games.</p>
            </div>
          ...
          <form className="grid gap-3 rounded-[26px] border border-white/12 bg-white/[0.055] p-4 backdrop-blur md:grid-cols-[1fr_auto]">
            <div className="flex min-h-12 items-center gap-3 rounded-xl bg-white px-4 text-[#0A1A3F]"><Search size={18} className="text-[#66718F]" /><input className="flex-1 bg-transparent text-sm font-semibold outline-none" name="q" placeholder={`Search ${state.name} schools, teams, athletes, coaches...`} type="search" /></div>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{state.schools.map((school) => <SchoolTile key={school.id} school={school} />)}</div>
