...
export default function SchoolsPage() {
  ...
  const title = selectedStateProfile?.tagline || (selectedState ? `${selectedState.name} schools` : "Choose a state");
  return (
    <PublicSiteShell variant="dark">
      <section className="relative min-h-screen overflow-hidden bg-[#061331] text-white">
        ...
        {selectedStateProfile?.coverImageUrl ? <img src={selectedStateProfile.coverImageUrl} alt="State cover" className="absolute inset-0 h-full w-full object-cover opacity-20" /> : null}
            ...
            <div className="min-w-0">
              <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div className="flex items-start gap-4">
                  {selectedStateProfile?.badgeImageUrl ? <img src={selectedStateProfile.badgeImageUrl} alt="State badge" className="h-20 w-28 rounded-2xl border border-white/15 object-cover" /> : null}
                  <div>
                    <div className="text-xs font-black uppercase tracking-[0.28em] text-[#F2C200]">Schools Directory</div>
                    <h1 className="mt-4 max-w-5xl text-4xl font-black tracking-tight sm:text-5xl">{title}</h1>
                    <p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-[#C8D6FF]">{selectedStateProfile?.bio || "Pick a state from the left rail. The center panel shows only that state and the schools inside it."}</p>
                  </div>
                </div>
