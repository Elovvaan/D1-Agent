    ...
    <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          setIsSaving(true);
          window.localStorage.setItem(LOCAL_AVATAR_KEY, previewSrc);
          window.dispatchEvent(new Event("myd1-avatar-updated"));
          setMessage("Profile picture saved and updated across the app.");
      ...
      >
        ...
        <Button variant="secondary">{isSaving ? "Saving..." : "Upload Photo"}</Button>
        {clientError || message ? <p className={clientError ? "sm:col-span-2 rounded-xl border border-[#FFD0D0] bg-[#FFF0F0] px-3 py-2 text-sm font-black text-[#B42318]" : "sm:col-span-2 rounded-xl border border-[#BDECCB] bg-[#EAF8F0] px-3 py-2 text-sm font-black text-[#17833F]"} role={clientError ? "alert" : "status"}>{clientError || message}</p> : null}
      </form>
    </div>
  );
}
