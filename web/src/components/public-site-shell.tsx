        ...
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span>
              ...
              <span className={`block text-xs font-semibold ${isDark ? "text-[#C8D6FF]" : "text-[#66718F]"}`}>{brandConfig.agentProductName} Platform</span>
            </span>
          </Link>

          <nav className="hidden items-center gap-1 md:flex">
            {navItems.map((item) => (
              <Link className={`rounded-xl px-3 py-2 text-sm font-black transition ${isDark ? "text-white hover:bg-white/10" : "text-[#0A1A3F] hover:bg-[#EAF0FF]"}`} href={item.href} key={item.href}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            <Link className={`hidden rounded-xl px-3 py-2 text-sm font-black sm:inline-flex ${isDark ? "text-[#AFC3FF] hover:bg-white/10" : "text-[#1B3FA0] hover:bg-[#EAF0FF]"}`} href="/sign-in">
              Sign In
            </Link>
      ...
      <footer className={`border-t ${isDark ? "border-white/12 bg-[#0A1A3F]" : "border-[#DDE3EC] bg-white"}`}>
        <div className="mx-auto grid max-w-[1440px] gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_auto] lg:px-8">
          <div>
            ...
            <p className={`mt-2 max-w-2xl text-sm font-semibold leading-6 ${isDark ? "text-[#C8D6FF]" : "text-[#66718F]"}`}>
          ...
          <div className={`flex flex-wrap items-center gap-3 text-sm font-black ${isDark ? "text-[#AFC3FF]" : "text-[#1B3FA0]"}`}>
            ...
            {connectedSocials.map((channel) => {
              ...
              return (
                <a className={`inline-flex items-center gap-1 rounded-xl px-2 py-1 transition ${isDark ? "hover:bg-white/10" : "hover:bg-[#EAF0FF]"}`} href={channel.url} key={channel.id} rel="noreferrer" target="_blank">
