...
import { usStateNameByCode, usStates, stateHref } from "@/lib/us-states";
...
function explicitStateCode(result: PublicDirectoryResult) { return /^[A-Z]{2}$/.test(String(result.stateCode ?? "")) ? result.stateCode : undefined; }
function resolveStateCode(result: PublicDirectoryResult) { return explicitStateCode(result) ?? normalizeState(`${result.title} ${result.detail}`); }
...
export function getNavigationGraph(): NavigationStateNode[] {
  ...
  const toResult = (node: OrgNode): PublicDirectoryResult => node.raw ?? { id: node.id, title: node.label, detail: node.detail, href: node.href, group: node.kind === "Game" ? "Games" : node.kind === "CoachRole" ? "Coaches" : node.kind === "AthleteRole" ? "Athletes" : "Organizations", typeLabel: node.sourceTypeLabel ?? node.kind, sourceLabel: "Public Record", stateCode: node.stateCode };
  const liveStates: NavigationStateNode[] = childNodes("country-us", "CONTAINS", "State").map((state) => ({ id: state.id, code: state.stateCode ?? "US", name: state.label, href: state.href, schools: childNodes(state.id, "CONTAINS", "School").map((school) => ({ id: school.raw?.id ?? school.id, title: school.label, detail: school.detail, href: school.href, teams: childNodes(school.id, "ACTIVE_IN", "Team").map((team) => ({ id: team.raw?.id ?? team.id, title: team.label, detail: team.detail, href: team.href, athletes: incoming(team.id, "PLAYS_FOR", "AthleteRole").map(toResult), coaches: incoming(team.id, "COACHES", "CoachRole").map(toResult), games: childNodes(team.id, "PARTICIPATED_IN", "Game").map(toResult) })), athletes: incoming(school.id, "PLAYS_FOR", "AthleteRole").map(toResult), coaches: incoming(school.id, "COACHES", "CoachRole").map(toResult), games: [] })) }));
  const liveCodes = new Set(liveStates.map((state) => state.code));
  const emptyStates: NavigationStateNode[] = usStates.filter((state) => !liveCodes.has(state.code)).map((state) => ({ id: `state-${state.code.toLowerCase()}`, code: state.code, name: state.name, href: stateHref(state.code), schools: [] }));
  return [...liveStates, ...emptyStates].sort((a, b) => a.name.localeCompare(b.name));
}
