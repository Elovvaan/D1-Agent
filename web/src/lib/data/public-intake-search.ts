...
type IntakeRecord = { id: string; sourceType?: string; state?: string; stateCode?: string; district?: string; school?: string; sport?: string; classYear?: string; sourceUrl?: string; sourceName?: string; notes?: string; athleteText?: string; extractedAthletes?: ExtractedAthlete[]; extractedCoaches?: ExtractedCoach[]; extractedCount?: number; extractedCoachCount?: number; sourceFetchStatus?: string; status?: string; createdAt?: string; pdf?: { name?: string; type?: string; size?: number } };
...
function normalizeStateCode(value?: string) { const code = String(value ?? "").trim().toUpperCase(); return /^[A-Z]{2}$/.test(code) ? code : undefined; }
...
function intakeToResults(item: IntakeRecord) {
  ...
  const stateCode = normalizeStateCode(item.stateCode || item.state);
  const baseDetail = [item.school, item.district, stateCode || item.state, item.sport, item.classYear ? `Class ${item.classYear}` : ""].filter(Boolean).join(" - ");
  ...
  addResult(results, { id: `intake-source-${slug(item.id)}`, title, detail: `${baseDetail || "Operations intake"} - ${athleteCount} athletes / ${coachCount} coaches extracted - ${item.sourceFetchStatus || "saved"}`, href: `/directory/sources/intake-${slug(item.id)}`, group: "Sources", typeLabel: item.sourceType ? `${item.sourceType} source` : "Intake Source", sourceLabel: "Public Record", sourceUrl: item.sourceUrl, importedAt: item.createdAt, stateCode });
  if (item.school) addResult(results, { id: `intake-school-${slug(item.school)}`, title: item.school, detail: [item.district, stateCode || item.state, item.sport, "Imported from Operations intake"].filter(Boolean).join(" - "), href: `/directory/school/intake-${slug(item.school)}`, group: "Schools", typeLabel: "School", sourceLabel: "Public Record", sourceUrl: item.sourceUrl, importedAt: item.createdAt, stateCode });
  const extractedAthletes = Array.isArray(item.extractedAthletes) ? item.extractedAthletes : [];
  for (const athlete of extractedAthletes) if (athlete.name) addResult(results, { id: `intake-athlete-${slug(athlete.name)}-${slug(item.id)}`, title: athlete.name, detail: athleteDetail(item, athlete), href: `/athletes/intake-${slug(athlete.name)}`, group: "Athletes", typeLabel: "Extracted Athlete", sourceLabel: "Public Record", sourceUrl: item.sourceUrl, importedAt: item.createdAt, confidence: 0.82, stateCode: normalizeStateCode(athlete.state) || stateCode });
  if (!extractedAthletes.length) for (const athlete of extractAthleteNames(item.athleteText)) addResult(results, { id: `intake-athlete-${slug(athlete)}-${slug(item.id)}`, title: athlete, detail: [item.sport, item.school, stateCode || item.state, item.classYear ? `Class ${item.classYear}` : "", "Claimable profile pending review"].filter(Boolean).join(" - "), href: `/athletes/intake-${slug(athlete)}`, group: "Athletes", typeLabel: "Claimable Athlete", sourceLabel: "Public Record", sourceUrl: item.sourceUrl, importedAt: item.createdAt, confidence: 0.7, stateCode });
  const extractedCoaches = Array.isArray(item.extractedCoaches) ? item.extractedCoaches : [];
  for (const coach of extractedCoaches) if (coach.name) addResult(results, { id: `intake-coach-${slug(coach.name)}-${slug(item.id)}`, title: coach.name, detail: coachDetail(item, coach), href: `/coaches/intake-${slug(coach.name)}`, group: "Coaches", typeLabel: "Extracted Coach", sourceLabel: "Public Record", sourceUrl: item.sourceUrl, importedAt: item.createdAt, confidence: 0.78, stateCode: normalizeStateCode(coach.state) || stateCode });
  return results;
}

function matches(result: PublicDirectoryResult, query: string) { return !query || [result.title, result.detail, result.group, result.typeLabel, result.sourceLabel, result.sourceUrl ?? "", result.stateCode ?? ""].join(" ").toLowerCase().includes(query); }
