...
export const usStateNameByCode: Record<string, string> = Object.fromEntries(usStates.map((state) => [state.code, state.name]));

export function stateHref(code: string) {
  return `/schools/${code.toLowerCase() === "us" ? "national" : code.toLowerCase()}`;
}
